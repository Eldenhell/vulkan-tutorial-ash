...
const WINDOW_TITLE: &str = "12. Rendering & Presentation";
...
struct VulkanApp {
    ...
    present_complete_semaphore: Option<vk::Semaphore>,
...
impl ApplicationHandler for VulkanApp {
    ...
    fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        ...
        let command_buffer = VulkanApp::create_command_buffer(&device, &command_pool);

        let (present_complete_semaphore, render_finished_semaphore, draw_fence) =
            VulkanApp::create_sync_objs(&device);
        ...
        self.draw_fence = Some(draw_fence);
    }
    ...
    fn suspended(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        unsafe {
            if let Some(device) = self.device.take() {
                ...
                if let Some(present_complete_semaphore) = self.present_complete_semaphore.take() {
                    device.destroy_semaphore(present_complete_semaphore, None);
                }

                if let Some(render_finished_semaphore) = self.render_finished_semaphore.take() {
                    device.destroy_semaphore(render_finished_semaphore, None);
                }

                if let Some(draw_fence) = self.draw_fence.take() {
                    device.destroy_fence(draw_fence, None);
                }
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                VulkanApp::draw_frame(
                    ...
                    &self.present_complete_semaphore.unwrap(),
                    &self.render_finished_semaphore.unwrap(),
...
impl VulkanApp {
    fn new(display_handle: &DisplayHandle) -> Self {
        ...
        let present_complete_semaphore = None;
        let render_finished_semaphore = None;
        let draw_fence = None;
    ...
    fn create_sync_objs(device: &ash::Device) -> (vk::Semaphore, vk::Semaphore, vk::Fence) {
        unsafe {
    ...
    fn draw_frame(
        ...
        unsafe {
            device
                .wait_for_fences(&[*draw_fence], true, u64::MAX)
                .expect("Failed to wait for fences");
            device
                .reset_fences(&[*draw_fence])
            ...
            device
                ...
                .expect("Failed to queue_wait_idle");
            ...
            device
                .queue_submit(*graphics_queue, &[submit_info], *draw_fence)
