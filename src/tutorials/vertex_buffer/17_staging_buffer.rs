...
const WINDOW_TITLE: &str = "17. Vertex staging buffer";
...
impl ApplicationHandler for VulkanApp {
    ...
    fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        ...
        let buffers = VulkanApp::create_vertex_buffer(
            &self.instance,
            &device,
            &self.physical_device,
            &command_pool,
        ...
        let (staging_buffer, staging_buffer_memory) = buffers[0];
        let (vertex_buffer, vertex_buffer_memory) = buffers[1];
        ...
        self.staging_buffer_memory = Some(staging_buffer_memory);
    ...
    fn suspended(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        unsafe {
            if let Some(device) = self.device.take() {
                ...
                if let Some(staging_buffer_memory) = self.staging_buffer_memory.take() {
...
impl VulkanApp {
    fn new(display_handle: &DisplayHandle) -> Self {
        ...
        let staging_buffer = None;
        let staging_buffer_memory = None;
    ...
    fn record_command_buffer(
        ...
        unsafe { device.cmd_draw(*buffer, VERTICES.len() as u32, 1, 0, 0) };
